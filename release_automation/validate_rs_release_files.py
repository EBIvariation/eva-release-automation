...
from release_automation.release_utils import collections_assembly_attribute_map, \
...
def get_unique_release_rs_ids(active_rs_ids_file, merged_rs_ids_file, deprecated_rs_ids_file, output_directory):
    ...
    run_command_with_output("Get active RS IDs", "gunzip -c {0} | grep -v ^# | cut -f3 | sed s/rs//g | {1} >> {2}"
                            .format(active_rs_ids_file, command_to_split_semicolon_separated_rs_ids, all_ids_file))
    run_command_with_output("Get merged RS IDs", "gunzip -c {0} | grep -v ^# | cut -f3 | sed s/rs//g | {1} >> {2}"
                            .format(merged_rs_ids_file, command_to_split_semicolon_separated_rs_ids, all_ids_file))
    run_command_with_output("Get deprecated RS IDs", "gunzip -c {0} | sed s/rs//g >> {1}"
...
def export_unique_rs_ids_from_mongo(mongo_database_handle, taxonomy_id, assembly_accession, mongo_unique_rs_ids_file):
    ...
    for collection in collections_assembly_attribute_map:
        if "clustered" in collection.lower():
            ...
            agg_pipeline = [{"$match": {"asm": assembly_accession}}]
            ...
            with open(collection_rs_ids_file, 'w+') as f:
                for r in collection_handle.aggregate(agg_pipeline):
                    f.write(str(r['accession']) + '\n')
...
def validate_rs_release_files(active_rs_ids_file, merged_rs_ids_file, deprecated_rs_ids_file, private_config_xml_file, profile, taxonomy_id, assembly_accession, output_directory):
    exit_code = -1

    mongo_host, mongo_user, mongo_pass = get_mongo_creds_for_profile(profile, private_config_xml_file)
    properties = get_properties_from_xml_file(profile, private_config_xml_file)
    db_name = properties.get('eva.accession.mongo.database', 'eva_accession_sharded')
    try:
        with MongoClient(host=mongo_host,username=mongo_user, password=mongo_pass) as client:
            db_handle = client[db_name]
            mongo_unique_rs_ids_file = os.path.join(output_directory, "{0}_mongo_unique_rs_ids.txt".format(assembly_accession))
            export_unique_rs_ids_from_mongo(db_handle, taxonomy_id, assembly_accession, mongo_unique_rs_ids_file)
            unique_release_rs_ids_file = get_unique_release_rs_ids(active_rs_ids_file, merged_rs_ids_file, deprecated_rs_ids_file, output_directory)
            missing_rs_ids_file = os.path.join(os.path.dirname(unique_release_rs_ids_file), assembly_accession + "_missing_ids.txt")
...
@click.option("--active_rs_ids_file", help="ex: /path/to/the active rs file", required=True)
@click.option("--merged_rs_ids_file", help="ex: /path/to/the merged rs file", required=True)
@click.option("--deprecated_rs_ids_file", help="ex: /path/to/the deprecated rs file", required=True)
@click.option("--private_config_xml_file", help="ex: /path/to/eva-maven-settings.xml", required=True)
@click.option("--profile", help="Maven profile to use, ex: internal", required=True)
@click.option("--taxonomy_id", help="ex: 9913", required=True)
@click.option("--assembly_accession", help="ex: GCA_000003055.6", required=True)
@click.option("--output_directory", required=True)
@click.command()
def main(active_rs_ids_file, merged_rs_ids_file, deprecated_rs_ids_file, private_config_xml_file, profile, taxonomy_id,
         assembly_accession, output_directory):
    logging_config.add_stdout_handler()
    validate_rs_release_files(active_rs_ids_file, merged_rs_ids_file, deprecated_rs_ids_file, private_config_xml_file, profile, taxonomy_id, assembly_accession,
                              output_directory)
